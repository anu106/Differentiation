...
function Calculator() {
  ...
  this.reset = function () {
    this.numResults = 0;
    this.graph = JXG.JSXGraph.initBoard("graph", {
      boundingbox: [-5, 5, 5, -5],
      axis: true
    });
    this.curves = [];
...
function onOptionChanged(e) {
  ...
  else calculator.curves[e.target.id].setAttribute({ visible: false });
}
...
function createGraphOption() {
  ...
  var text = "\\(f";
  for (let i = 1; i < calculator.numResults; ++i) text += "'";
  text += "(" + wrt + ")\\)";
...
function onOperationChanged() {
  ...
  document.getElementById("options").style.display = simplify
    ? "none"
    : "inline";
}
